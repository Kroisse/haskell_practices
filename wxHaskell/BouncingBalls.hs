...
-- the maximal height is at most maximal y minus the radius of a ball.
maxH :: Int
maxH = maxY - radius
...
ballsFrame = do
	-- a list of balls, where each ball is represnted
	...
	set p [ on click         := dropBall vballs p
		...
		, on (charKey '-') := set t [ interval :~ (*2) ]  -- increase interval
  ...
  where
	...
	drawBall dc pt = (dc # circle) pt radius []
